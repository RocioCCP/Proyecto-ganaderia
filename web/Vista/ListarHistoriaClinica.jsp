<%-- 
    Document   : ListarHistoriaClinica
    Created on : 20/03/2024, 07:35:08 PM
    Author     : Rocio Castillo
--%>

<%@page import="Modelo.HistoriaClinica"%>
<%@page import="DaoPersistencia.DaoProcedimientos"%>
<%@page import="Modelo.Procedimientos"%>
<%@page import="DaoPersistencia.DaoMedicamentos"%>
       ...
       <nav class="navbar navbar-expand-md navbar-dark bg-dark border-3 fixed-top border-bottom" id="menu">
        ...
        <div class="row">           
            ...
            <div class="card col-sm-4 mb-4  mt-5 sticky-top">
                ...
                <h4 class="text-secondary elegant-font mt-4"><b>Formulario Historia Clinica</b></h4> 

                <%--  Inicio Formulario --%>

                <form action="ControladorHistoriaClinica" method="POST" autocomplete="off" class="custom-form">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group text-left">
                                ...
                                <select class="form-control" id="medica" value="${User.getMedicamentosId()}" name="txtMedica" required>
                                    <option value="0">Seleccione medicamento</option>
                                    <% List<Medicamentos> medicam = DaoMedicamentos.listar();
                                        if (medicam != null) {
                                            for (Medicamentos medicamentos : medicam) {
                                                if (medicamentos != null) {%>
                                    <option value="<%=medicamentos.getIdMedicamento()%>"><%=medicamentos.getMedicamento()%></option>
                                    <% }
                                            }
                                        }%>
                                </select>
                                <small id="medicaError" class="error form-text text-muted">
                                    Campo es obligatorio <span class="error">*</span>
                                </small>
                            </div>

                            <div class="form-group text-left">
                                <label for="procd">Procedimiento</label>
                                <select class="form-control" id="proced" value="${User.getProcedimientosId()}" name="txtProced" required>
                                    <option value="0">Seleccione procedimiento</option>
                                    <% List<Procedimientos> procedim = DaoProcedimientos.listar();
                                        if (procedim != null) {
                                            for (Procedimientos procedimientos : procedim) {
                                                if (procedimientos != null) {%>
                                    <option value="<%=procedimientos.getIdProcedimientos()%>"><%=procedimientos.getTipoprocedimiento()%></option>
                                    <% }
                                            }
                                        }%>
                                </select>
                                <small id="procedError" class="error form-text text-muted">
                                    Campo es obligatorio <span class="error">*</span>
                                </small>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group text-left">
                                <label for="fech">Fecha Procedimiento:</label>
                                <input type="date" class="form-control" id="fechapro" value="${User.getFechaproceso()}" name="txFechaProc" required>
                            ...
                            <div class="form-group text-left">
                                <label for="recom">Recomendaciones</label>
                                <input type="text" class="form-control" id="recomed" value="${User.getRecomendaciones()}" name="txRecomenda" placeholder="Ingrese Informacion" required>
                            ...
                            <div class="form-group text-left">
                                ...
                                <input type="text" readonly="" class="form-control" id="idedi" value="${User.getIdHistoriaClinica()}" name="txt">
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-sm-7 mb-4 mt-5 sticky-top">
                <h4  class="text-secondary elegant-font mt-4"><b > Lista Historia Clinica</b></h4> 
                ...
                <div class=" table-container ml-3 md-3 table-responsive" >
                    <table  id="miTabla" class="table table-striped table-hover sticky-top">
                        ...
                        <tbody>
                            <%
                                List<HistoriaClinica> Lista = (List<HistoriaClinica>) request.getAttribute("listaHistoriaClinica");
                                for (HistoriaClinica historiaclinica : Lista) {%>
                            <tr>
                                <td><%= historiaclinica.getIdHistoriaClinica()%></td>
                                <td><%=DaoVaca.obtenerunNombreVaca(historiaclinica.getVacaId())%></td>
                                <td><%=DaoVaca.obtenerunPlacaVacaporid(historiaclinica.getVacaId())%></td>
                                <td><%= historiaclinica.getFechaproceso()%></td>       
                                <td><%=DaoProcedimientos.obtenerunNombreProcedimientos(historiaclinica.getProcedimientosId())%></td>
                                <td><%=DaoMedicamentos.obtenerunNombreMedicamentos(historiaclinica.getMedicamentosId())%></td>
                                <td><%= historiaclinica.getRecomendaciones()%></td>
                                <td>
                                    <div class="btn-group" role="group" aria-label="Acciones">

                                    <div class="mt-2">

                                         <a href="ControladorHistoriaClinica?accion=editar&id=<%= historiaclinica.getIdHistoriaClinica()%>"  class="btn btn-outline-light btn-success"> 
                                            <!-- id  de un usuario se lo envio al controlador  en variable id -->  
                                            <i class="fas fa-pencil-alt"></i> <!-- Ícono de lápiz -->  
                                        </a>
                                        <a href="ControladorHistoriaClinica?accion=eliminar&id=<%= historiaclinica.getIdHistoriaClinica()%>"
                ...
                <div class="form-group mt-4 text-right">
                    <a href="ControladorHistoriaClinica?accion=listar" class="btn btn-primary mr-2"style="background-color: #0C4F5A; border-color:#0C4F5A;">
